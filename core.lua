-- StormdownnHub V1 - Tema claro padrão com opção para modo escuro
...
-- Estado do tema: true = modo claro (padrão), false = modo escuro
local isLightTheme = true

-- Função para aplicar tema claro ou escuro no hub
local function applyTheme(frame)
    if isLightTheme then
        frame.BackgroundColor3 = Color3.fromRGB(245, 245, 245) -- branco quase puro
        for _, child in pairs(frame:GetChildren()) do
            if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
                child.TextColor3 = Color3.fromRGB(30,30,30) -- texto escuro
                if child:IsA("TextButton") or child:IsA("TextBox") then
                    child.BackgroundColor3 = Color3.fromRGB(230,230,230) -- botão claro
        ...
        frame.BackgroundColor3 = Color3.fromRGB(20, 20, 20) -- escuro
        for _, child in pairs(frame:GetChildren()) do
            if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
                child.TextColor3 = Color3.fromRGB(230,230,230) -- texto claro
                if child:IsA("TextButton") or child:IsA("TextBox") then
                    child.BackgroundColor3 = Color3.fromRGB(40, 40, 40) -- botão escuro
                    child.BorderSizePixel = 0
                end
            elseif child:IsA("Frame") then
                applyTheme(child)
...
-- Aplicar tema inicial claro no login
applyTheme(loginFrame)

-- Hub GUI (invisível até login)
...
-- Scripts e callbacks simulados (substitua URLs pelos reais)
local scripts = {
...
-- Botão flutuante para abrir/fechar hub
...
local function loadSettings()
    ...
    -- Aplica tema ao frame inicialmente
    applyTheme(frame)
...
settingsBtn.Text = "⚙️"
settingsBtn.TextColor3 = Color3.fromRGB(30,30,30)
...
enterBtn.MouseButton1Click:Connect(function()
    if input.Text == "stormdownn" then
        loginGui:Destroy()
        hubGui.Enabled = true
        applyTheme(hubFrame) -- aplicar tema ao abrir
    else
